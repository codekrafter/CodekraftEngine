...
#include <unistd.h>
    ...
    {
        LOG(LOG_ERROR) << "Could not find UUID of asset type: '" << typeid(a).name() << "'";
        return 0x0000;
    }*/
    };
    ...
    /*void AssetManager::load(std::string name)
{
    ...
    void AssetManager::savef01(std::string name)
    {
        ...
        {
            LOG(LOG_ERROR) << "Failed to open asset file for saving";
            LOG(LOG_ERROR) << "Error: " << strerror(errno);
        }
        of.close();
    ...
    void AssetManager::loadf(std::string name)
    {
        ...
        if (is.is_open())
        {
            ...
            if (is.read(buffer.data(), size))
            {
                ...
                default:
                    ...
                    LOG(LOG_ERROR) << "Unknown version: 0x" << ss.str();
                    return;
                }
            }
        }
        ...
        {
            LOG(LOG_ERROR) << "Failed to open asset file for loading";
            LOG(LOG_ERROR) << "Error: " << strerror(errno);
            return;
        }
        /*
 ...
 */
    ...
    void AssetManager::loadf01(std::vector<unsigned char> d, std::string cname)
    {
        ...
        for (HeaderEntry h : header)
        {
            ...
            if (!(crc == h.hash))
            {
                LOG(LOG_ERROR) << "Error while loading asset: checksums do not match";
            }
    ...
    /*size_t AssetManager::getSize(std::string name)
{
    ...
    {
        LOG(LOG_ERROR) << "Could not find asset for name: '" << name << "'";
        return 0;
    }
};*/
