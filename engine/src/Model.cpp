...
{
    ar(textures_loaded, directory, gammaCorrection);
    for (Mesh *m : *meshes)
    {
        ar(*m);
    }
...
{
    std::cout << "started to draw model, will draw " << meshes->size() << "meshes" << std::endl;
    return;
    for (unsigned int i = 0; i < meshes->size(); i++)
    {
        std::cout << "in model drawing loop" << std::endl;
        //if(i > 100) {break;}
        //std::cout << "drawing mesh " << i << "/" << meshes->size() << std::endl;
        meshes->at(i)->Draw(shader);
        //meshes[i]->Draw(shader);
    }
    std::cout << "done drawing for this frame" << std::endl;
    return;
}
...
{
    std::cout << "processing " << node->mNumMeshes << " meshes" << std::endl;
    meshes->resize(meshes->size() + node->mNumMeshes);
    ...
    std::cout << "done pushing back meshes, total meshes: " << meshes->size() << " meshes" << std::endl;
